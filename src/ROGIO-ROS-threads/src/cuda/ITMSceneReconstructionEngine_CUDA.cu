...
#include "ITMSceneReconstructionEngineShared.h"
...
namespace {
    ...
    __global__ void integrateIntoScene_device(ITMVoxel_d *localVBA,
...
void ITMSceneReconstructionEngine_CUDA::computeMapBlock(ITMScene *scene, Vector3s *blockPos_device) {
...
void ITMSceneReconstructionEngine_CUDA::ResetScene(ITMScene *scene) {
    ...
    ITMVoxel_d *voxelBlocks_ptr = scene->localVBA.GetVoxelBlocks();
    memsetKernel<ITMVoxel_d>(voxelBlocks_ptr, ITMVoxel_d(), numBlocks * blockSize);
...
void ITMSceneReconstructionEngine_CUDA::AllocateSceneFromDepth(ITMScene *scene,
    ...
    //为第一次命中的block申请hash条目和在GPU上分配内存位置（即localVBA上）
    ...
    cudaDeviceSynchronize();
    std::cout<<"depth alloc size: "<<*alloc + *alloc_extra<<std::endl;
    ...
    std::cout<<"swapout size: "<<*swapout_num<<std::endl;
    cudaDeviceSynchronize();
...
void ITMSceneReconstructionEngine_CUDA::AllocateScene(ITMScene *scene,
                                                              const cv::cuda::GpuMat &depth_map,
                                                              const Eigen::Matrix4d &pose,
                                                              ITMRenderState_VH *renderState_vh,
                                                              Vector4f camera_intrinsic,
    ...
    std::cout<<"Local submap found block: "<<*found_num<<std::endl;
    cudaMemcpy(alloc, alloc_device, sizeof(int), cudaMemcpyDeviceToHost);
    cudaMemcpy(alloc_extra, alloc_extra_device, sizeof(int), cudaMemcpyDeviceToHost);
...
void ITMSceneReconstructionEngine_CUDA::SwapAllBlocks(ITMScene *scene,
                                                              ITMRenderState_VH *renderState_vh,
                                                              int *NeedToSwapIn) {
    ITMHashEntry *hashTable = scene->index.GetEntries();
    ITMHashSwapState *swapStates = scene->globalCache != NULL ? scene->globalCache->GetSwapStates(true) : 0;
...
void ITMSceneReconstructionEngine_CUDA::IntegrateIntoScene(ITMScene *scene,
                                                                   const cv::cuda::GpuMat &depth_map,
                                                                   const cv::cuda::GpuMat &rgb,
                                                                   const Eigen::Matrix4d &pose_inv,
                                                                   ITMRenderState_VH *renderState_vh,
                                                                   Vector4f depth_intrinsic,
                                                                   Vector4f color_intrinsic,
                                                                   const Eigen::Matrix4d& extrinic_from_d_2_rgb,
                                                                   float truncation_distance) {
    float voxelSize = scene->voxel_resolution;
    ...
    M_rgb = extrinic_from_d_2_rgb.cast<float>()*M_d;
    projParams_d = depth_intrinsic;
    projParams_rgb = color_intrinsic;
    ...
    std::cout<<"Visible Entries : "<<renderState_vh->noVisibleEntries<<std::endl;

    integrateIntoScene_device<true> <<<gridSize, cudaBlockSize >>>
...
void ITMSceneReconstructionEngine_CUDA::showHashTableAndVoxelAllocCondition(ITMScene *scene,ITMRenderState_VH *renderState_vh)
{
...
namespace {
    ...
    __global__ void integrateIntoScene_device(ITMVoxel_d *localVBA,
        ...
        ITMVoxel_d *localVoxelBlock = &(localVBA[currentHashEntry.ptr * SDF_BLOCK_SIZE3]);
        ...
        if (localVoxelBlock[locId].w_depth == maxW) return;
        Vector4f pt_model;
        ...
        ComputeUpdatedVoxelInfo::compute
