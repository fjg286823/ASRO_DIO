...
#include "ITMSwappingEngineShared.h"
...
namespace {
    ...
    __global__ void integrateOldIntoActiveData_device(ITMVoxel_d *localVBA,
                                                      ITMHashSwapState *swapStates,
                                                      ITMHashSwapState *States,
                                                      ITMVoxel_d *syncedVoxelBlocks_local,
                                                      int *neededEntryIDs_local,
                                                      ITMHashEntry *hashTable,
                                                      int maxW);


    __global__ void integrateDataCpuAndDevice(ITMVoxel_d *localVBA,
                                              ITMHashSwapState *swapStates,
                                              ITMHashSwapState *States,
                                              ITMVoxel_d *syncedVoxelBlocks_local,
                                              int *neededEntryIDs_local,
    ...
    __global__ void moveActiveDataToTransferBuffer_device(ITMVoxel_d *syncedVoxelBlocks_local,
...
ITMSwappingEngine_CUDA::ITMSwappingEngine_CUDA(void) {
...
int ITMSwappingEngine_CUDA::LoadFromGlobalMemory(ITMScene *scene) {
    ITMGlobalCache *globalCache = scene->globalCache;

    ITMHashSwapState *swapStates = globalCache->GetSwapStates(true);
    ITMHashSwapState *States = globalCache->GetStates(true);

    ITMVoxel_d *syncedVoxelBlocks_local = globalCache->GetSyncedVoxelBlocks(true);
    ...
    ITMVoxel_d *syncedVoxelBlocks_global = globalCache->GetSyncedVoxelBlocks(false);
    ...
    if (noNeededEntries > 0) {
        ...
        memset(syncedVoxelBlocks_global, 0, noNeededEntries * SDF_BLOCK_SIZE3 * sizeof(ITMVoxel_d));
        ...
        for (int i = 0; i < noNeededEntries; i++) {
            ...
            if (globalCache->HasStoredData(entryId)) {
                ...
                memcpy(syncedVoxelBlocks_global + i * SDF_BLOCK_SIZE3, globalCache->GetStoredVoxelBlock(entryId),
                       SDF_BLOCK_SIZE3 * sizeof(ITMVoxel_d));
            }
        }
        ...
        DWIOcudaSafeCall(cudaMemcpy(syncedVoxelBlocks_local, syncedVoxelBlocks_global,
                                    sizeof(ITMVoxel_d) * SDF_BLOCK_SIZE3 * noNeededEntries, cudaMemcpyHostToDevice));
    }
    return noNeededEntries_origin;
}


void ITMSwappingEngine_CUDA::IntegrateGlobalIntoLocal(ITMScene *scene,
...
void ITMSwappingEngine_CUDA::TransferGlobalMap(ITMScene *scene) {
    ITMGlobalCache *globalCache = scene->globalCache;
...
void ITMSwappingEngine_CUDA::SaveToGlobalMemory(ITMScene *scene, ITMRenderState_VH *renderState) {
    ITMGlobalCache *globalCache = scene->globalCache;
    ...
    if (noNeededEntries > 0) {
        ...
        DWIOcudaSafeCall(cudaMemcpy(syncedVoxelBlocks_global, syncedVoxelBlocks_local,
                                    sizeof(ITMVoxel_d) * SDF_BLOCK_SIZE3 * noNeededEntries, cudaMemcpyDeviceToHost));
...
void ITMSwappingEngine_CUDA::MoveVoxelToGlobalMemorey(ITMScene *scene, ITMRenderState_VH *renderState) {
    ITMGlobalCache *globalCache = scene->globalCache;
...
namespace {
    ...
    __global__ void moveActiveDataToTransferBuffer_device(ITMVoxel_d *syncedVoxelBlocks_local,
        ...
        ITMVoxel_d *dstVB = syncedVoxelBlocks_local + blockIdx.x * SDF_BLOCK_SIZE3;
        ITMVoxel_d *srcVB = localVBA + hashEntry.ptr * SDF_BLOCK_SIZE3;

        int vIdx = threadIdx.x + threadIdx.y * SDF_BLOCK_SIZE + threadIdx.z * SDF_BLOCK_SIZE * SDF_BLOCK_SIZE;
        dstVB[vIdx] = srcVB[vIdx];
        srcVB[vIdx] = ITMVoxel_d();
    ...
    __global__ void integrateOldIntoActiveData_device(ITMVoxel_d *localVBA,
        ...
        ITMVoxel_d *srcVB = syncedVoxelBlocks_local + blockIdx.x * SDF_BLOCK_SIZE3;
        ITMVoxel_d *dstVB = localVBA + hashTable[entryDestId].ptr * SDF_BLOCK_SIZE3;
    ...
    __global__ void integrateDataCpuAndDevice(ITMVoxel_d *localVBA,
        ...
        CombineVoxelInformation::compute(srcVB[vIdx], dstVB[vIdx], maxW);
