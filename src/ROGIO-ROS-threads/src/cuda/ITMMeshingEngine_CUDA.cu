...
__global__ void meshScene_device(ITMMesh::Triangle *triangles, unsigned int *noTriangles_device, float factor, int noTotalEntries,
	int noMaxTriangles, const Vector4s *visibleBlockGlobalPos, const ITMVoxel_d *localVBA, const ITMHashEntry *hashTable);
...
ITMMeshingEngine_CUDA::ITMMeshingEngine_CUDA(void)
{
...
void ITMMeshingEngine_CUDA::MeshScene(ITMMesh *mesh, const ITMScene *scene,int& TotalTriangles)
{
	ITMMesh::Triangle *triangles = mesh->triangles;
	const ITMVoxel_d *localVBA = scene->localVBA.GetVoxelBlocks();
	...
	{ // mesh used voxel blocks
		...
		DWIOcudaSafeCall(cudaMemset(noTriangles_device, 0, sizeof(unsigned int)));
		meshScene_device << <gridSize, cudaBlockSize >> >(triangles, noTriangles_device, factor, noTotalEntries, noMaxTriangles,
