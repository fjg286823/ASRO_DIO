...
namespace DWIO
{
	...
	public:
		...
		static const uint noMaxTriangles_default = (SDF_BUCKET_NUM + SDF_EXCESS_LIST_SIZE) * 20;//* 32 * 16
		uint noMaxTriangles;

		Triangle *triangles;//应该是存放顶点的
		...
		explicit ITMMesh(MemoryDeviceType memoryType, uint maxTriangles = noMaxTriangles_default)
		{
			...
			DWIOcudaSafeCall(cudaMalloc((void **)&triangles, noMaxTriangles * sizeof(Triangle)));
		...
		void WriteSTL(const char *fileName)
		{
			...
			{
				cudaMallocHost((void **)&cpu_triangles, noMaxTriangles * sizeof(Triangle));
				DWIOcudaSafeCall(cudaMemcpy(cpu_triangles,triangles,sizeof(Triangle)*noTotalTriangles,cudaMemcpyDeviceToHost));
			...
			Triangle *triangleArray = cpu_triangles;
		...
		void savePly(const std::string &filename)
		{
			...
			if (!file_out.is_open())
				return;
			Triangle *cpu_triangles; bool shoulDelete = false;
			if (memoryType == MEMORYDEVICE_CUDA)
			{
				cudaMallocHost((void **)&cpu_triangles, noMaxTriangles * sizeof(Triangle));
				DWIOcudaSafeCall(cudaMemcpy(cpu_triangles,triangles,sizeof(Triangle)*noTotalTriangles,cudaMemcpyDeviceToHost));
		...
		void SwapAndClear()
		{
			cudaMallocHost((void **)&cpu_triangles, noMaxTriangles * sizeof(Triangle));
			DWIOcudaSafeCall(cudaMemcpy(cpu_triangles,triangles,sizeof(Triangle)*noTotalTriangles,cudaMemcpyDeviceToHost));
			DWIOcudaSafeCall(cudaMemset(triangles, 0, this->noMaxTriangles * sizeof(Triangle)));
